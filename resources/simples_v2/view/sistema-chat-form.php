...
if(isset($_GET['alterar'])){
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_sistema_chat_contrato', "WHERE id_sistema_chat_contrato = $id");
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_sistema_chat" style="cursor: pointer;"><strong>Sistema de chat</strong></a></li>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> sistema de chat:</h3>
                    <?php if(isset($_GET['alterar'])){
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a class=\"a_modalAguarde\" href=\"/api/ajax?class=SistemaChat.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')){ return false; }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a>";
                        
                        echo "</div>";
                        } 
                        ?>
                </div>
                <form method="post" action="/api/ajax?class=SistemaChat.php" id="sistema_chat_form" style="margin-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <label>*Sistema:</label>
                                    <select name='id_tipo_sistema_chat' class="form-control input-sm">
                                        <?php
                                            $sistema_chat = DBRead('', 'tb_tipo_sistema_chat', "ORDER BY nome ASC");
                                            if($sistema_chat){
                                                echo "<option></option>";
                                                foreach($sistema_chat as $conteudo){
                                                    $idSistema = $conteudo['id_tipo_sistema_chat'];
                                                    $nomeSistema = $conteudo['nome'];
                                                    $selected = $dados[0]['id_tipo_sistema_chat'] == $idSistema ? "selected" : "";
                                ...
                                <div class="form-group">
                                        <label>Link de Acesso:</label>
                                            ...
                                            <?php
                                                ...
                                                $sel_link_acesso[$id_link_acesso] = 'selected';
                                                foreach($dados_link as $dado){ ?>
                                                    <option value="<?= $dado['id_link_acesso']?>" <?= $sel_link_acesso[$dado['id_link_acesso']]?> ><?= $dado['nome']?></option>
                                                <?php } ?>
                                            </select>
                                </div>
                            </div> -->
                                        ...
                                        <div class='table-responsive'>
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                ...
                                                <tbody>
                                                    <?php
                                                    if($operacao == "alterar"){
                                                        $dados_sistema_chat_acesso = DBRead('', 'tb_sistema_chat_acesso', "WHERE id_sistema_chat_contrato = $id");
                                                        foreach($dados_sistema_chat_acesso as $conteudo){
                                                            echo "<tr class='linha_sistema_chat'>";
                    ...
                    <div class="panel-footer">
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                <?php
                                $pagina = 'sistema-chat-form';

                                if($ativacao == 1){

                                    echo "<input type='hidden' id='tela_sistema_proximo' value='0' />";
                                    $dados_pular = DBRead('', 'tb_informacao_geral_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                    $id_dados_pular = $dados_pular[0]['id_informacao_geral_contrato'];

                                    $dados_voltar = DBRead('', 'tb_sistema_gestao_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa ORDER BY id_sistema_gestao_contrato desc");
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=informacoes-gerais-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $(document).on('submit', '#sistema_chat_form', function () {
        ...
        if(!$("tr.linha_sistema_chat").length){
