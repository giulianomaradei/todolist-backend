...
if(isset($_GET['alterar'])) {
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_manual_contrato', "WHERE id_manual_contrato = $id");

    if($dados){
        $tituloPainel = 'Alterar';
        $operacao = 'alterar';

        $manual = $dados[0]['manual'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_manual" style="cursor: pointer;"><strong>Manual</strong></a></li>
        ...
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> manual:</h3>
                    <?php if (isset($_GET['alterar'])) {
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=Manual.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; } ?>
                </div>
                <form method="post" action="/api/ajax?class=Manual.php" id="manual_form" style="margin-bottom: 0;">
                    ...
                    <div class="panel-body" style="padding-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>*Manual:</label>
                                    <textarea rows="12" cols="50" id="manual" required name="manual" class="form-control ckeditor"><?= $manual ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-12" style="text-align: center">
                                ...
                                <?php
                                    ...
                                    if($ativacao == 1){
                                        ...
                                        $dados_pular = DBRead('', 'tb_plano_cliente_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_pular = $dados_pular[0]['id_plano_cliente_contrato'];

                                        $dados_voltar = DBRead('', 'tb_ura_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_voltar = $dados_voltar[0]['id_ura_contrato'];
                                        ...
                                        //}else{
                                            echo "<button type='button' class='btn btn-primary' data-toggle='modal' data-target='#modalSalvar'><i class='fa fa-floppy-o'></i> Salvar</button>";


                                            ?>
                                                <div class="modal fade" id="modalSalvar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                Deseja permanecer na mesma tela?
                                                            </div>
...
<script>
    ...
    $('#pular').on('click', function(){
        ...
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=plano-cliente-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=ura-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $(document).on('submit', '#manual_form', function () {
        ...
        if(!manual || manual == 0){
            alert("Deve-se preencher o manual!");
            return false;
        }
        modalAguarde();
    });
