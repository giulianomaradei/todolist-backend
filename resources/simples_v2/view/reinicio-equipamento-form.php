...
if(isset($_GET['alterar'])) {
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_reinicio_equipamento_contrato', "WHERE id_reinicio_equipamento_contrato = $id");
    ...
    if($dados){
        ...
        $dados_reinicio = DBRead('', 'tb_reinicio_equipamento', "WHERE id_reinicio_equipamento_contrato = $id");
        $equipamento = $dados_reinicio['equipamento'];
        $tempo = $dados_reinicio['tempo'];
        $observacao = $dados_reinicio['observacao'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_tempo_reinicio" style="cursor: pointer;"><strong>Tempo de reinicio de equipamentos</strong></a></li>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> tempo de reinício de equipamentos:</h3>
                    <?php if(isset($_GET['alterar'])){
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=ReinicioEquipamento.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; } ?>
                </div>
                <form method="post" action="/api/ajax?class=ReinicioEquipamento.php" id="reinicio_form" style="margin-bottom: 0;">
                    ...
                    <div class="panel-body" style="padding-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading clearfix">
                                        <h3 class="panel-title text-left pull-left">Equipamentos:</h3>
                                    </div>
                                    <div class="panel-body">
                                        <div class='table-responsive'>
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                <thead>
                                                    <tr>
                                                        <th class='col-md-5'>*Tipo de Equipamento</th>
                                                        <th class='col-md-1'>*Tempo(seg.)</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    if($operacao == "alterar"){
                                                        ...
                                                        foreach($dados_reinicio as $conteudo){
                                                            ...
                                                            echo "<tr class='linha_equipamento'>";
                                                                echo "<td>";
                                                                    echo "<select required class='form-control input-sm equipamento' name='equipamento[]'>";
                                                                        foreach($tipo_equipamento as $conteudoEquipamento){
                                                                            $selected = $conteudo['equipamento'] == $conteudoEquipamento['id_tipo_equipamento'] ? "selected" : "";
                                                                            echo "<option value='".$conteudoEquipamento['id_tipo_equipamento']."' ".$selected.">".$conteudoEquipamento['descricao']."</option>";
                                                                        }
                                                                    echo "</select>";
                                                                echo "</td>";

                                                                echo "<td><input required class='form-control input-sm number-int tempo' name='tempo[]' value='".$conteudo['tempo']."' /></td>";
                    ...
                    <div class="panel-footer">
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                $pagina = 'reinicio-equipamento-form';

                                if($ativacao == 1){

                                    $dados_pular = DBRead('', 'tb_equipamento_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                    $id_dados_pular = $dados_pular[0]['id_equipamento_contrato'];
                                    
                                    $dados_voltar = DBRead('', 'tb_catalogo_equipamento_qi_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                    $id_dados_voltar = $dados_voltar[0]['id_catalogo_equipamento_qi_contrato'];
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=acesso-equipamento-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=equipamento-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $("#adiciona-equipamento").on('click', function(){
        $("tbody").append("<tr class='linha_equipamento'><td><select required class='form-control input-sm equipamento' name='equipamento[]'><option value='1'>ADSL</option><option value='2'>Cabo</option><option value='3'>Fibra</option><option value='4'>Rádio</option><option value='5'>VDSL</option><option value='6'>UTP</option></select></td><td><input required class='form-control input-sm number_int tempo' name='tempo[]' /></td><td><textarea class='form-control input-sm observacao' name='observacao[]'></textarea></td><td><button class='center-block btn btn-danger btn-sm removeLinha' role='button'><i class='fa fa-trash-o' aria-hidden='true'></i></button></td></tr>");
    ...
    $(document).on('submit', '#reinicio_form', function(){
        var naoSalva = 0;
        var id_contrato_plano_pessoa = $("#id_contrato_plano_pessoa").val();
        var equipamento = $(".equipamento").val();
        var tempo = $(".tempo").val();
        if(!equipamento){
            alert("Deve-se escrever um equipamento válido!");
            return false;
        }
        if(!tempo){
            alert("Deve-se ter um tempo válido!");
            return false;
        }
        ...
        if(!$("tr.linha_equipamento").length){
            alert("Deve haver pelo menos um equipamento configurado!");
            return false;
        }

        $("tr.linha_equipamento").each(function(index_primeiro){
            equipamento_primeiro = $(this).find(".equipamento").val();
            $("tr.linha_equipamento").each(function(index_segundo){
                equipamento_segundo = $(this).find(".equipamento").val();
                if(parseInt(index_primeiro) != parseInt(index_segundo)){
                    if(equipamento_primeiro == equipamento_segundo){
                        ++naoSalva;
                    }
                }
            });
        ...
        if(naoSalva >= 1){
            alert("Não é possível inserir dois parâmetros com o mesmo equipamento!");
            return false;
        }
        modalAguarde();
    });
