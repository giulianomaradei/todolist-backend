...
if(isset($_GET['alterar'])){
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_ura_contrato', "WHERE id_ura_contrato = $id");
    ...
    if($dados){
        ...
        $dados_ura = DBRead('', 'tb_ura', "WHERE id_ura_contrato = $id");
        $numero = $dados_ura['numero'];
        $descricao = $dados_ura['descricao'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_ura" style="cursor: pointer;"><strong>URA</strong></a></li>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> URA:</h3>
                    <?php if (isset($_GET['alterar'])) {
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=Ura.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; } ?>
                </div>
                <form method="post" action="/api/ajax?class=Ura.php" id="ura_form" style="margin-bottom: 0;">
                    ...
                    <div class="panel-body" style="padding-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading clearfix">
                                        <h3 class="panel-title text-left pull-left">Números:</h3>
                                    </div>
                                            ...
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                ...
                                                <tbody>
                                                    ...
                                                    if($operacao == "alterar"){
                                                        ...
                                                        foreach($dados_ura as $conteudo){
                                                            echo "<tr class='linha_numero'>";
                                                                echo "<td><input required class='form-control input-sm number_int numero' name='numero[]' value='".$conteudo['numero']."' /></td>";
                                                                echo "<td><textarea required class='form-control input-sm descricao' name='descricao[]'>".$conteudo['descricao']."</textarea></td>";
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        <td></td>
                                                        <td></td>
                                                        <td><button type="button" class='center-block btn btn-warning btn-sm' id='adiciona-numero' role='button'><i class='fa fa-plus' aria-hidden='true'></i></button></td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                            ...
                            <div class="col-md-12" style="text-align: center">
                                ...
                                <?php
                                    ...
                                    if($ativacao == 1){
                                        ...
                                        $dados_pular = DBRead('', 'tb_manual_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_pular = $dados_pular[0]['id_manual_contrato'];

                                        $dados_voltar = DBRead('', 'tb_parametros', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_voltar = $dados_voltar[0]['id_parametros'];
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=manual-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=parametro-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $("#adiciona-numero").on('click', function(){
        $("tbody").append("<tr class='linha_numero'><td><input required class='form-control input-sm number_int numero' name='numero[]' /></td><td><textarea required class='form-control input-sm descricao' name='descricao[]'></textarea></td><td><button class='center-block btn btn-danger btn-sm removeLinha' role='button'><i class='fa fa-trash-o' aria-hidden='true'></i></button></td></tr>");
    ...
    $(document).on('submit', '#ura_form', function () {
        var naoSalva = 0;
        var id_contrato_plano_pessoa = $("#id_contrato_plano_pessoa").val();
        var numero = $(".numero").val();
        var descricao = $(".descricao").val();
        if(!numero){
            alert("Deve-se descrever um número válido!");
            return false;
        }
        if(!descricao){
            alert("Deve-se escrever uma descrição válida!");
            return false;
        }
        ...
        if(!$("tr.linha_numero").length){
            alert("Deve haver pelo menos um número configurado!");
            return false;
        }
        $("tr.linha_numero").each(function(index_primeiro){

            numero_primeiro = $(this).find(".numero").val();

            $("tr.linha_numero").each(function(index_segundo){

                numero_segundo = $(this).find(".numero").val();

                if(parseInt(index_primeiro) != parseInt(index_segundo)){

                    if(parseInt(numero_primeiro) == parseInt(numero_segundo)){
                        ++naoSalva;
                    }
                }
            });
        ...
        if(naoSalva >= 1){
            alert("Não é possível inserir dois parâmetros com o mesmo número!");
            return false;
        }
        modalAguarde();
    });
