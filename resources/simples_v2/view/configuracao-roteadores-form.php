...
isset($ativacao = (int)$_GET['ativacao']) ? $ativacao : '';
...
if(isset($_GET['alterar'])){
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_configuracao_roteadores_contrato', "WHERE id_configuracao_roteadores_contrato = $id");
    ...
    if($dados){
        ...
        $dados_configuracao = DBRead('', 'tb_configuracao_roteadores', "WHERE id_configuracao_roteadores = $id");
        $nome = $dados_configuracao['nome'];
        $configuracao = $dados_configuracao['configuracao'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_conexao_cabo" style="cursor: pointer;"><strong>Conexões de cabos</strong></a></li>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> conexões de cabos:</h3>
                    <?php if (isset($_GET['alterar'])) {
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=ConfiguracaoRoteadores.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; 
                    } ?>
                </div>
                <form method="post" action="/api/ajax?class=ConfiguracaoRoteadores.php" id="configuracao_form" style="margin-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea <?=$focus?> name="observacao" class="form-control"><?= $observacao; ?></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading clearfix">
                                        <h3 class="panel-title text-left pull-left">Configurações:</h3>
                                    </div>
                                            ...
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th class='col-md-5'>*Configuração</th>
                                                ...
                                                <tbody>
                                                    <?php
                                                    if($operacao == "alterar"){
                                                        $dados_configuracao_roteadores = DBRead('', 'tb_configuracao_roteadores', "WHERE id_configuracao_roteadores_contrato = $id");
                                                        foreach($dados_configuracao_roteadores as $conteudo){
                                                            echo "<tr class='linha_configuracao'>";
                                                                echo "<td><input required class='form-control input-sm nome' name='nome[]' value='".$conteudo['nome']."' /></td>";
                                                                echo "<td><textarea required class='form-control input-sm configuracao' name='configuracao[]'>".$conteudo['configuracao']."</textarea></td>";
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        <td></td>
                                                        <td></td>
                                                        <td><button type="button" class='center-block btn btn-warning btn-sm' id='adiciona-configuracao' role='button'><i class='fa fa-plus' aria-hidden='true'></i></button></td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                if($ativacao == 1){
                                    ...
                                    $dados_pular = DBRead('', 'tb_catalogo_equipamento_qi_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                    $id_dados_pular = $dados_pular[0]['id_catalogo_equipamento_qi_contrato'];

                                    $dados_voltar = DBRead('', 'tb_prazo_retorno_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa AND tipo = 3");
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=equipamento-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=prazo-retorno-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato+"&tela=3";
        }else{
    ...
    $("#adiciona-configuracao").on('click', function(){
        $("tbody").append("<tr class='linha_configuracao'><td><input required class='form-control input-sm nome' name='nome[]' /></td><td><textarea required class='form-control input-sm configuracao' name='configuracao[]'></textarea></td><td><button class='center-block btn btn-danger btn-sm removeLinha' role='button'><i class='fa fa-trash-o' aria-hidden='true'></i></button></td></tr>");
        $(".nome").focus();
    });
    ...
    $(document).on('submit', '#configuracao_form', function(){
        var naoSalva = 0;
        var id_contrato_plano_pessoa = $("#id_contrato_plano_pessoa").val();
        var nome = $(".nome").val();
        var configuracao = $(".configuracao").val();
        if(!nome){
            alert("Deve-se escrever um nome válido!");
            return false;
        }
        if(!configuracao){
            alert("Deve-se escrever uma configuração válida!");
            return false;
        }
        ...
        if(!$("tr.linha_configuracao").length){
            alert("Deve haver pelo menos uma configuração configurado!");
            return false;
        }
        $("tr.linha_configuracao").each(function(index_primeiro){

            nome_primeiro = $(this).find(".nome").val();

            $("tr.linha_configuracao").each(function(index_segundo){

                nome_segundo = $(this).find(".nome").val();

                if(parseInt(index_primeiro) != parseInt(index_segundo)){

                    if(nome_primeiro == nome_segundo){
                        ++naoSalva;
                    }
                }
            });
        ...
        if(naoSalva >= 1){
            alert("Não é possível inserir duas configurações com o mesmo nome!");
            return false;
        }
        modalAguarde();
    });
