...
if(isset($_GET['alterar'])){
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_sistema_gestao_contrato', "WHERE id_sistema_gestao_contrato = $id");
    ...
    if($dados){
        ...
        $dados_sistema_gestao_acesso = DBRead('', 'tb_sistema_gestao_contrato', "WHERE id_sistema_gestao_contrato = $id");
        $usuario = $dados_sistema_gestao_acesso['usuario'];
        $senha = $dados_sistema_gestao_acesso['senha'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        <li class="active"><a id="li_sistema_gestao" style="cursor: pointer;"><strong>Sistema de gestão</strong></a></li>
        <li class="colorir-breadcrumbs-info"><a id="li_sistema_chat" style="cursor: pointer;">Sistema de chat</a></li>
        <li class="colorir-breadcrumbs-info"><a id="li_informacao_geral" style="cursor: pointer;">Informações gerais e de registro</a></li>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> sistema de gestão:</h3>
                    <?php if(isset($_GET['alterar'])){
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a class=\"a_modalAguarde\" href=\"/api/ajax?class=SistemaGestao.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')){ return false; }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a>";
                        
                        echo "</div>";
                        } 
                        ?>
                </div>
                <form method="post" action="/api/ajax?class=SistemaGestao.php" id="sistema_gestao_form" style="margin-bottom: 0;">
                    ...
                    <div class="panel-body" style="padding-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <label>*Sistema:</label>
                                    <select name='id_tipo_sistema_gestao' class="form-control input-sm">
                                        <?php
                                            ...
                                            if($sistema_gestao){
                                                ...
                                                foreach($sistema_gestao as $conteudo){
                                                    ...
                                                    $selected = $dados[0]['id_tipo_sistema_gestao'] == $idSistema ? "selected" : "";
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input name='observacao' value='<?= $observacao ?>' class='form-control input-sm'>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class="col-md-6">
                                <div class='form-group'>
                                    <label>*Link:</label>
                                    <input name="link" type="text" id="link" class="form-control input-sm" value="<?= $link; ?>" autocomplete="off" required />
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                        <label>Link de Acesso:</label>
                                            <select class="form-control input-sm" name="id_link_acesso" id="id_link_acesso" required> 
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading clearfix">
                                        <h3 class="panel-title text-left pull-left">Usuários:</h3>
                                    </div>
                                        ...
                                        <div class='table-responsive'>
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                ...
                                                <tbody>
                                                    <?php
                                                    if($operacao == "alterar"){
                                                        $dados_sistema_gestao_acesso = DBRead('', 'tb_sistema_gestao_acesso', "WHERE id_sistema_gestao_contrato = $id");
                                                        foreach($dados_sistema_gestao_acesso as $conteudo){
                                                            echo "<tr class='linha_sistema_gestao'>";
                                                                echo "<td><input class='form-control input-sm usuario' name='usuario[]' required value='".$conteudo['usuario']."' /></td>";
                                                                echo "<td><input class='form-control input-sm senha' name='senha[]' required value='".$conteudo['senha']."' /></td>";
                    ...
                    <div class="panel-footer">
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                <?php
                                $pagina = 'sistema-gestao-form';

                                if($ativacao){

                                    $dados_sistema = DBRead('', 'tb_sistema_gestao_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                    $id_dados_sistema = $dados_sistema[0]['id_sistema_gestao_contrato'];
                                    $proximo = DBRead('', 'tb_sistema_gestao_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa AND id_sistema_gestao_contrato > $id");
                                    $id_proximo = $proximo[0]['id_sistema_gestao_contrato'];

                                    echo "<input type='hidden' value='$id_proximo' name='id_proximo' id='id_proximo' />";

                                    if($id_proximo && $operacao != 'inserir'){
                                        echo "<input type='hidden' id='tela_sistema_proximo' value='1' />";
                                        $id_dados_pular = $id_proximo;
                                    }else{
                                        echo "<input type='hidden' id='tela_sistema_proximo' value='0' />";
                                        $dados_pular = DBRead('', 'tb_sistema_chat_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_pular = $dados_pular[0]['id_sistema_chat_contrato'];
                                    }

                                    $voltar = DBRead('', 'tb_sistema_gestao_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa AND id_sistema_gestao_contrato < $id");
                                    
                                    $id_voltar = $voltar[0]['id_sistema_gestao_contrato'];

                                    echo "<input type='hidden' value='$id_voltar' name='id_voltar' id='id_voltar' />";

                                    if($id_voltar && $operacao != 'inserir'){
                                        echo "<input type='hidden' id='tela_sistema_voltar' value='1' />";
                                    ...
                                    echo "<button class='btn btn-primary btn-comando-ativacao' name='salvar' id='ok' type='submit'><i class='fa fa-arrow-right' aria-hidden='true'></i> Salvar e continuar</button>";
                                    ...
                                    echo "<button class='btn btn-primary btn-comando-ativacao' value='1' name='adicionar_sistema' id='adicionar-sistema' type='submit'><i class='fa fa-plus' aria-hidden='true'></i> Adicionar sistema de gestão</button>";
...
<script>
    ...
    var tela_sistema_voltar = $("#tela_sistema_voltar").val();
    var voltar = $("#id_voltar").val();

    var tela_sistema_proximo = $("#tela_sistema_proximo").val();
    var proximo = $("#id_proximo").val();

    $('#pular').on('click', function(){

        if(tela_sistema_proximo == 1){
            if(idDadosPular){
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-chat-form&alterar="+proximo+"&ativacao=1&id_contrato="+idContrato;
            }else{
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-chat-form&ativacao=1&id_contrato="+idContrato;
            }
        }else{
            if(idDadosPular){
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-chat-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
            }else{
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-chat-form&ativacao=1&id_contrato="+idContrato;
            }
        }
    ...
    $('#voltar').on('click', function(){
        if(tela_sistema_voltar == 1){
            if(idDadosVoltar){
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-gestao-form&alterar="+voltar+"&ativacao=1&id_contrato="+idContrato;
            }else{
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-gestao-form&ativacao=1&id_contrato="+idContrato;
            }
        }else{
            if(idDadosVoltar){
                window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sistema-gestao-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
            }else{
    ...
    $("#adiciona-usuario").on('click', function(){
        $("tbody").append("<tr class='linha_sistema_gestao'><td><input required class='form-control input-sm usuario' name='usuario[]' /></td><td><input required class='form-control input-sm senha' name='senha[]' /></td><td><button class='center-block btn btn-danger btn-sm removeLinha' role='button'><i class='fa fa-trash-o' aria-hidden='true'></i></button></td></tr>");
        $(".usuario").focus();
    });
    ...
    $(document).on('submit', '#sistema_gestao_form', function () {
        ...
        if(!$("tr.linha_sistema_gestao").length){
            alert("Deve haver pelo menos um usuário configurado!");
            return false;
        }
        if(!usuario){
            alert("Deve-se cadastrar um usuário!");
            return false;
        }
        if(!senha){
            alert("Deve-se cadastrar uma senha!");
            return false;
        }

        if(!id_link_acesso){
            alert("Deve-se selecionar um link de acesso!");
            return false;
        }
