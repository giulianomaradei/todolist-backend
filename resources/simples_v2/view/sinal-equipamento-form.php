...
if(isset($_GET['alterar'])){
    ...
    $dados = DBRead('', 'tb_sinal_equipamento_contrato', "WHERE id_sinal_equipamento_contrato = $id");
    ...
    if($dados){
        ...
        $dados_sinal = DBRead('', 'tb_sinal_equipamento', "WHERE id_sinal_equipamento_contrato = $id");
        $nome = $dados_sinal['nome'];
        $intensidade_sinal = $dados_sinal['intensidade_sinal'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_sinal_equipamento" style="cursor: pointer;"><strong>Sinais dos equipamentos</strong></a></li>
        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> sinais dos equipamentos:</h3>
                    <?php if (isset($_GET['alterar'])) {
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=SinalEquipamento.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; 
                    } ?>
                </div>
                <form method="post" action="/api/ajax?class=SinalEquipamento.php" id="sinal_equipamento_form" style="margin-bottom: 0;">
                                            ...
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th class='col-md-3'>*Intensidade do sinal</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    if($operacao == "alterar"){
                                                        ...
                                                        foreach($dados_sinal as $conteudo){
                                                            echo "<tr class='linha_sinal'>";
                                                                echo "<td><input class='form-control input-sm nome' name='nome[]' required value='".$conteudo['nome']."' /></td>";
                                                                echo "<td><input class='form-control input-sm intensidade_sinal' required name='intensidade_sinal[]' value='".$conteudo['intensidade_sinal']."' /></td>";
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        <td></td>
                                                        <td></td>
                                                        <td><button type="button" class='center-block btn btn-warning btn-sm' id='adiciona-sinal' role='button'><i class='fa fa-plus' aria-hidden='true'></i></button></td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                <?php
                                    ...
                                    $pagina = 'acesso-equipamento-form';

                                    if($ativacao == 1){

                                        $dados_pular = DBRead('', 'tb_velocidade_minima_encaminhar_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_pular = $dados_pular[0]['id_velocidade_minima_encaminhar_contrato'];

                                        $dados_voltar = DBRead('', 'tb_equipamento_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_voltar = $dados_voltar[0]['id_equipamento_contrato'];
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=velocidade-minima-encaminhar-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=acesso-equipamento-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $("#adiciona-sinal").on('click', function(){
        $("tbody").append("<tr class='linha_sinal'><td><input required class='form-control input-sm nome' name='nome[]' /></td><td><input required class='form-control input-sm intensidade_sinal' name='intensidade_sinal[]' /></td><td><textarea class='form-control input-sm' name='observacao[]'></textarea></td><td><button class='center-block btn btn-danger btn-sm removeLinha' role='button'><i class='fa fa-trash-o' aria-hidden='true'></i></button></td></tr>");
    ...
    $(document).on('submit', '#sinal_equipamento_form', function () {
        var id_contrato_plano_pessoa = $("#id_contrato_plano_pessoa").val();
        var descricao = $(".nome").val();
        var intensidade = $(".intensidade_sinal").val();
        if(!descricao){
            alert("Deve-se escrever uma descrição válido!");
            return false;
        }
        if(!intensidade){
            alert("Deve-se ter uma intensidade válida!");
            return false;
        }
