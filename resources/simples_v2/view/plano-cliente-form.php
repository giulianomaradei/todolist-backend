...
if(isset($_GET['alterar'])){
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_plano_cliente_contrato', "WHERE id_plano_cliente_contrato = $id");
    ...
    if($dados){
        ...
        $dados_atributos = DBRead('', 'tb_plano_cliente', "WHERE id_plano_cliente_contrato = $id");

        $dados_contrato = DBRead('', 'tb_contrato_plano_pessoa a', "INNER JOIN tb_plano b ON a.id_plano = b.id_plano INNER JOIN tb_pessoa c ON a.id_pessoa = c.id_pessoa WHERE id_contrato_plano_pessoa = '$id_contrato'", "a.*, b.cod_servico, b.nome AS 'plano', c.nome AS 'nome_pessoa', b.id_plano");
...
<div class="container-fluid">
    ...
    <li class="colorir-breadcrumbs-success"><a id="li_sistema_gestao" style="cursor: pointer; color: #3c763d;">Sistema de gestão</a></li>
        ...
        <li class="colorir-breadcrumbs-success"><a id="li_parametro" style="cursor: pointer; color: #3c763d;">Parâmetros</a></li>
        <li class="colorir-breadcrumbs-success"><a id="li_ura" style="cursor: pointer; color: #3c763d;">URA</a></li>
        <li class="colorir-breadcrumbs-success"><a id="li_manual" style="cursor: pointer; color: #3c763d;">Manual</a></li>
        <li class="active"><a id="li_plano" style="cursor: pointer;"><strong>Planos</strong></a></li>
    </ol>
    <?php
    endif;
    ?>
        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> planos:</h3>
                    <?php if (isset($_GET['alterar'])) {
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=PlanoCliente.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; } ?>
                </div>
                <form method="post" action="/api/ajax?class=PlanoCliente.php" id="plano_cliente_form" style="margin-bottom: 0;">
                            ...
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading clearfix">
                                        ...
                                        <?php
                                        if($ativacao != 1 && $operacao == 'alterar'){
<<<<<<< HEAD
                                            echo "<div id='csv' class='panel-title text-right pull-right'><a href='/api/iframe?token=<?php echo $request->token ?>&view=importar-csv-planos-cliente-form&contrato=$id_contrato&id_plano=$id'><button type='button' title='Deve-se selecionar um contrato antes!' id='btn-csv' class='btn btn-xs btn-warning'><i class='fa fa-plus'></i> Importar CSV</button></a></div>";
                                        }else{
                                            echo "<div id='csv' class='panel-title text-right pull-right'><a href='/api/iframe?token=<?php echo $request->token ?>&view=importar-csv-planos-cliente-form&contrato=$id_contrato_plano_pessoa&id_plano=-1'><button type='button' title='Deve-se selecionar um contrato antes!' id='btn-csv' class='btn btn-xs btn-warning'><i class='fa fa-plus'></i> Importar CSV</button></a></div>";
=======
                                            echo "<div id='csv' class='panel-title text-right pull-right'><a href='/api/iframe?token=".$request->token."&view=importar-csv-planos-cliente-form&contrato=$id_contrato&id_plano=$id'><button type='button' title='Deve-se selecionar um contrato antes!' id='btn-csv' class='btn btn-xs btn-warning'><i class='fa fa-plus'></i> Importar CSV</button></a></div>";
                                        }else{
                                            echo "<div id='csv' class='panel-title text-right pull-right'><a href='/api/iframe?token=".$request->token."&view=importar-csv-planos-cliente-form&contrato=$id_contrato_plano_pessoa&id_plano=-1'><button type='button' title='Deve-se selecionar um contrato antes!' id='btn-csv' class='btn btn-xs btn-warning'><i class='fa fa-plus'></i> Importar CSV</button></a></div>";
>>>>>>> 5cfe18e1256c22b2f7585786435771d80b84680b
                                        }
                                        ?>
                                    </div>
                                    <div class="panel-body">
                                        ...
                                        <div class='table-responsive'>
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                <thead>
                                                    <tr>
                                                        <th class='col-md-2'>*Download (Mbps)</th>
                                                        <th class='col-md-2'>*Upload (Mbps)</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    if($operacao == "alterar"){
                                                        ...
                                                        foreach($dados_atributos as $conteudo){
                                                            
                                                            echo "<tr class='linha_plano'>";

                                                                echo "<td>";
                                                                    echo "<input type='text' required class='form-control input-sm download number_float' name='download[]' value='".$conteudo['download']."' />";
                                                                echo "</td>";

                                                                echo "<td>";
                                                                    echo "<input type='text' required class='form-control input-sm upload number_float' name='upload[]' value='".$conteudo['upload']."' />";
                                                                echo "</td>";

                                                                echo "<td>";
                                                                    echo "<textarea required class='form-control input-sm descricao' name='descricao[]'>".$conteudo['descricao']."</textarea>";
                                                                echo "</td>";

                                                                echo "<td>";
                                                                    echo "<textarea class='form-control input-sm observacao' name='observacao[]'>".$conteudo['observacao']."</textarea>";
                                                                echo "</td>";
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        <td></td>
                                                        <td></td>
                                                        <td><button type="button" class='center-block btn btn-warning btn-sm' id='adiciona-plano' role='button'><i class='fa fa-plus' aria-hidden='true'></i></button></td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                <?php
                                    ...
                                    $pagina = 'plano-cliente-form';

                                    if($ativacao == 1){
                                        
                                        $dados_voltar = DBRead('', 'tb_manual_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_voltar = $dados_voltar[0]['id_manual_contrato'];
...
<script>
    ...
    $("#btn-csv").on("click", function(){
        btn = $(this);
        if($("#busca_contrato").val() == ""){
            alert("Deve-se selecionar um contrato antes");
            return false;
        }
    });
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=manual-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $("#adiciona-plano").on('click', function(){

        $("tbody").append("<tr class='linha_plano'><td><input type='text' class='form-control input-sm download number_float' required name='download[]' value='' /></td><td><input type='text' class='form-control input-sm upload number_float' required name='upload[]' value='' /></td><td><textarea class='form-control input-sm descricao' name='descricao[]' required></textarea></td><td><textarea class='form-control input-sm observacao' name='observacao[]'></textarea></td><td><button class='center-block btn btn-danger btn-sm removeLinha' role='button'><i class='fa fa-trash-o' aria-hidden='true'></i></button></td></tr>");
    });

    $(document).on('click', '.removeLinha', function(){
        if(confirm('Deseja excluir o plano?')){
        ...
        if(busca != "" && busca.length >= 2){
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#csv').html("<a href='/api/iframe?token=<?php echo $request->token ?>&view=importar-csv-planos-cliente-form&contrato="+data[0].id_contrato_plano_pessoa+"'><button type='button' class='btn btn-xs btn-warning'><i class='fa fa-plus'></i> Importar CSV</button></a>");
                }
            });
        }
    }
    ...
    $(document).on('submit', '#plano_cliente_form', function(){
        ...
        if(!download){
            alert("Deve-se indicar um download válido!");
            return false;
        }
        if(!descricao){
            alert("Deve-se indicar uma descrição válida!");
            return false;
        }
        ...
        if(!$("tr.linha_plano").length){
            alert("Deve haver pelo menos um tipo configurado!");
            return false;
        }
        $("tr.linha_plano").each(function(index_primeiro){
            download_primeiro = $(this).find(".download").val().toUpperCase();;
            upload_primeiro = $(this).find(".upload").val().toUpperCase();
            descricao_primeiro = $(this).find(".descricao").val().toLowerCase();
            observacao_primeiro = $(this).find(".observacao").val().toLowerCase();

            $("tr.linha_plano").each(function(index_segundo){
                download_segundo = $(this).find(".download").val().toUpperCase();
                upload_segundo = $(this).find(".upload").val().toUpperCase();
                descricao_segundo = $(this).find(".descricao").val().toLowerCase();
                observacao_segundo = $(this).find(".observacao").val().toLowerCase();

                if(index_primeiro != index_segundo){
                    if(download_primeiro == download_segundo && upload_primeiro == upload_segundo && descricao_primeiro == descricao_segundo && observacao_primeiro == observacao_segundo){
                        ++naoSalva;
                    }
                }
            });
        ...
        if(naoSalva >= 1){
            alert("Não é possível inserir dois planos com a mesma configuração!");
            return false;
        }
