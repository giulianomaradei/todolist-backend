...
if(isset($_GET['alterar'])){
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_equipamento_contrato', "WHERE id_equipamento_contrato = $id");
    if($dados){
        ...
        $dados_equipamento_acesso = DBRead('', 'tb_equipamento_acesso', "WHERE id_equipamento_contrato = $id");
        $usuario = $dados_equipamento_acesso['usuario'];
        $senha = $dados_equipamento_acesso['senha'];
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_acesso_equipamento" style="cursor: pointer;"><strong>Acesso a equipamentos</strong></a></li>
        ...
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> acessos a equipamentos:</h3>
                    ...
                    <?php if(isset($_GET['alterar'])){
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=AcessoEquipamento.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); } else { modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>";
                    } ?>
                </div>
                <form method="post" action="/api/ajax?class=AcessoEquipamento.php" id="acesso_equipamento_form" style="margin-bottom: 0;">
                    ...
                    <div class="panel-body" style="padding-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading clearfix">
                                        <h3 class="panel-title text-left pull-left">Usuários de acesso a equipamento:</h3>
                                    </div>
                                        ...
                                        <div class='table-responsive'>
                                            <table class='table table-bordered' style='font-size: 14px;'>
                                                ...
                                                <tbody>
                                                    ...
                                                    if($operacao == "alterar"){
                                                        $dados_equipamento_acesso = DBRead('', 'tb_equipamento_acesso', "WHERE id_equipamento_contrato = $id");
                                                        foreach($dados_equipamento_acesso as $conteudo){
                                                            echo "<tr class='linha_acesso_equipamentos'>";
                                                                echo "<td><input required class='form-control input-sm usuario' name='usuario[]' value='".$conteudo['usuario']."' /></td>";
                                                                echo "<td><input required class='form-control input-sm senha' name='senha[]' value='".$conteudo['senha']."' /></td>";
                            ...
                            <div class="col-md-12" style="text-align: center">
                                <input type="hidden" id="operacao" value="<?= $id; ?>" name="<?= $operacao; ?>" />
                                    ...
                                    if($ativacao == 1){
                                        ...
                                        $dados_pular = DBRead('', 'tb_sinal_equipamento_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_pular = $dados_pular[0]['id_sinal_equipamento_contrato'];


                                        $dados_voltar = DBRead('', 'tb_reinicio_equipamento_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa");
                                        $id_dados_voltar = $dados_voltar[0]['id_reinicio_equipamento_contrato'];
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=sinal-equipamento-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=reinicio-equipamento-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $(document).on('submit', '#acesso_equipamento_form', function () {
        ...
        var senha = $(".senha").val();
        if(!usuario){
            alert("Deve-se escrever um usuário válido!");
            return false;
        }
        if(!senha){
            alert("Deve-se escrever uma senha válida!");
            return false;
        }
        ...
        if(!$("tr.linha_acesso_equipamentos").length){
