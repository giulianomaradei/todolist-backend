...
if(isset($_GET['alterar'])) {
    ...
    $id = (int)$_GET['alterar'];
    $dados = DBRead('', 'tb_plantonista_contrato', "WHERE id_plantonista_contrato = $id");
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a id="li_plantonista" style="cursor: pointer;"><strong>Plantonistas</strong></a></li>
        ...
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title text-left pull-left"><?= $tituloPainel ?> plantonistas:</h3>
                    <?php if (isset($_GET['alterar'])) {
                        ...
                        echo "<div class=\"panel-title text-right pull-right\"><a href=\"/api/ajax?class=Plantonista.php?excluir= $id&exclui-ativacao=$exclui_ativacao&id-contrato=$id_contrato_plano_pessoa&token=". $request->token ."\" onclick=\"if (!confirm('Tem certeza que deseja excluir o registro?')) { return false; }else{ modalAguarde(); }\"><button class=\"btn btn-xs btn-danger\"><i class=\"fa fa-trash\"></i> Excluir</button></a></div>"; } ?>
                </div>
                <form method="post" action="/api/ajax?class=Plantonista.php" id="plantonista_form" style="margin-bottom: 0;">
                    ...
                    <div class="panel-body" style="padding-bottom: 0;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>*Tabela:</label>
                                    <textarea rows="12" cols="50" id="tabela" required name="tabela" class="form-control ckeditor"><?= $tabela ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="panel-footer">
                        <div class="row">
                            <div class="col-md-12" style="text-align: center">
                                ...
                                $pagina = 'plantonista-form';

                                if($ativacao == 1){

                                    $dados_pular = DBRead('', 'tb_horario_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa AND tipo = 1");
                                    $id_dados_pular = $dados_pular[0]['id_horario_contrato'];

                                    $dados_voltar = DBRead('', 'tb_localizacao_contrato', "WHERE id_contrato_plano_pessoa = $id_contrato_plano_pessoa ORDER BY id_localizacao_contrato desc");
                                    $id_dados_voltar = $dados_voltar[0]['id_localizacao_contrato'];
...
<script>
    ...
    $('#pular').on('click', function(){
        if(idDadosPular){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=horario-form&alterar="+idDadosPular+"&ativacao=1&id_contrato="+idContrato+"&tela=1";
        }else{
    ...
    $('#voltar').on('click', function(){
        ...
        if(idDadosVoltar){
            window.location.href = "/api/iframe?token=<?php echo $request->token ?>&view=localizacao-form&alterar="+idDadosVoltar+"&ativacao=1&id_contrato="+idContrato;
        }else{
    ...
    $(document).on('submit', '#plantonista_form', function () {
        ...
        if(!id_contrato_plano_pessoa || id_contrato_plano_pessoa == 0){
            alert("Deve-se cadastrar pelo menos um plantonista!");
            return false;
        }
        if(!tabela){
            alert("Deve-se cadastrar plantonistas!");
            return false;
        }
        modalAguarde();
    });
