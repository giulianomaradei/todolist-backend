...
$dados = DBRead('','tb_faq_categoria',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_faq_categoria',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_faq_categoria'];
        $nome = $conteudo['nome'];

        $dados_faq = DBRead('', 'tb_faq', "WHERE id_faq_categoria = $id");

        echo "<tr>";    
            echo "<td onclick=\"window.location='/api/iframe/?token=".$request->token."&view=categoria-form&alterar=$id'\" style='cursor: pointer'>$nome</td>";
            if($dados_faq){
                echo "<td class=\"text-center\"><a href='/api/iframe/?token=".$request->token."&view=faq-categoria-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
            }else{
                echo "<td class=\"text-center\"><a href='/api/iframe/?token=".$request->token."&view=faq-categoria-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/api/ajax/?token=".$request->token."&class=FaqCategoria.php&excluir=$id\" title='Excluir' 
                onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
            }
        echo "</tr>";
    }
    echo "</tbody>";
