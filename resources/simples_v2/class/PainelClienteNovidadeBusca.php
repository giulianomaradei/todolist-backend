...
$filtros_query  = "WHERE descricao LIKE '%$letra%' AND status = 1 ORDER BY data ASC";
...
$dados = DBRead('','tb_painel_novidades',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_painel_novidades',$filtros_query." LIMIT $inicio,$maximo", "id_painel_novidades, data,descricao");
if(!$dados){
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-8\">Descricao</th>";
    echo "<th class=\"col-md-3\">Data</th>";
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_painel_novidades'];
        $data = converteData($conteudo['data']);
        $descricao = $conteudo['descricao'];

        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=painel-cliente-novidade-form&alterar=$id'\" style='cursor: pointer'>".limitarTexto($descricao, 100)."</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=painel-cliente-novidade-form&alterar=$id'\" style='cursor: pointer'>$data</td>";
        
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=painel-cliente-novidade-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/PainelClienteNovidade.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
