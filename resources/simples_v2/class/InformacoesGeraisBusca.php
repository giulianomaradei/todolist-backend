...
$dados = DBRead('','tb_informacao_geral_contrato a', $filtros_query, "COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
//
$dados = DBRead('', 'tb_informacao_geral_contrato a',$filtros_query." LIMIT $inicio, $maximo", "a.*, b.*, d.nome AS 'nome', c.nome AS 'plano', c.cod_servico, b.nome_contrato");
if(!$dados){
    ...
    echo "<th class=\"col-md-1\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_informacao_geral_contrato'];
        ...
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=informacoes-gerais-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=informacoes-gerais-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/InformacoesGerais.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; }else{ modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
