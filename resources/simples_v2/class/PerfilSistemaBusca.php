...
$dados = DBRead('','tb_perfil_sistema',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_perfil_sistema',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-11\">Nome do Perfil</th>";
    echo "<th class=\"col-md-1 text-center\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_perfil_sistema'];
        $nome = $conteudo['nome'];     
        ...
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=perfil-sistema-form&alterar=$id'\" style='cursor: pointer'>$nome</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=perfil-sistema-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/PerfilSistema.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
