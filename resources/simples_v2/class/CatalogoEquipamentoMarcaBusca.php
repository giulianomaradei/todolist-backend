...
$filtros_query  = "WHERE nome LIKE '%".$letra."%' ORDER BY nome ASC";
...
$dados = DBRead('','tb_catalogo_equipamento_marca',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_catalogo_equipamento_marca',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
	...
	echo "<thead>";
	echo "<tr>";
	echo "<th class=\"col-md-10\">Marca</th>";
	echo "<th class=\"col-md-2 text-center\">Opções</th>";
	echo "</tr>";
	echo "</thead>";
	echo "<tbody>";
	foreach ($dados as $conteudo) {
		$id = $conteudo['id_catalogo_equipamento_marca'];
		...
		if($conteudo['status'] == 0){
			$status = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/CatalogoEquipamentoMarca.php?ativar=$id\" title='Ativar' onclick=\"if (!confirm('Ativar " . addslashes($nome) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-upload' style='color:#228B22;'></i></a>";

		}else if($conteudo['status'] == 1){
			$status = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/CatalogoEquipamentoMarca.php?desativar=$id\" title='Desativar' onclick=\"if (!confirm('Desativar " . addslashes($nome) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-download' style='color:#FFA500;'></i></a>";
		}



		echo "<td> $nome</td>";
		echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=catalogo-equipamento-marca-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>".$status."</td>";
		echo "</tr>";
	}
	echo "</tbody>";
	echo "</table>";
