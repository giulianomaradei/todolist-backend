...
$filtros_query  = "WHERE name LIKE '7%' AND (callerid LIKE '%$letra%' OR name LIKE '%$letra%') ORDER BY callerid ASC";
...
$dados = DBRead('snep','peers',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('snep','peers',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<th class=\"col-md-2\">Ramal</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $nome = utf8_decode($conteudo['callerid']);
        $ramal = $conteudo['name'];
        echo "<tr>";    
        echo "<td>$nome</td>";
        echo "<td>$ramal</td>";
