...
$filtros_query  = "INNER JOIN tb_contrato_plano_pessoa b ON a.id_contrato_plano_pessoa = b.id_contrato_plano_pessoa INNER JOIN tb_plano c ON b.id_plano = c.id_plano INNER JOIN tb_pessoa d ON b.id_pessoa = d.id_pessoa WHERE d.nome LIKE '%$letra%' ".$filtro_plano." ORDER BY d.nome ASC";
...
$dados = DBRead('','tb_parametros a', $filtros_query, "COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
//
$dados = DBRead('', 'tb_parametros a',$filtros_query." LIMIT $inicio, $maximo", "a.*, b.*, d.nome AS 'nome', c.nome AS 'plano', c.cod_servico, b.nome_contrato");
if(!$dados){
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-10\">Cliente - Contrato</th>";
    ...
    foreach($dados as $conteudo){
        ...
        $email_envio = $conteudo['email_envio'];

        if($email_envio && $email_envio != ' ' && $email_envio != ''){
            $btn_email = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/Parametro.php?teste_email=$id\" title='Testar e-mails' onclick=\"if (!confirm('Testar: ".$email_envio."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-envelope' style='color:#5cb85c;'></i></a>";
        }else{
            $btn_email = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"#\" title='Testar e-mails' onclick=\"alert('Não possui e-mail cadastrado!')\"><i class='fa fa-envelope' style='color:#b92c28;'></i></a>";
        }

        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=parametro-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=parametro-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>$btn_email&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/Parametro.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
