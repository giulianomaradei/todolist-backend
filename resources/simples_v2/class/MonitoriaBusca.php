...
if($parametros['mes']){
    $mes = "AND a.data_referencia LIKE '%-".$parametros['mes']."-01' ";
}

if($parametros['ano']){
    $ano = "AND a.data_referencia LIKE '".$parametros['ano']."%-01' ";
}

$filtros_query = "INNER JOIN tb_usuario b ON a.id_usuario = b.id_usuario INNER JOIN tb_pessoa c ON b.id_pessoa = c.id_pessoa WHERE a.id_monitoria_resultado $id_usuario $mes $ano ORDER BY a.data_referencia DESC";
...
$dados = DBRead('','tb_monitoria_resultado a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
$dados = DBRead('', 'tb_monitoria_resultado a', $filtros_query." LIMIT $inicio,$maximo","a.*, b.id_usuario, c.nome");
...
if (!$dados) {
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_monitoria_resultado'];
        $nome = $conteudo['nome'];
        ...
        echo "<td class=\"text-center\">
                <a href='/api/iframe?token=<?php echo $request->token ?>&view=monitoria-resultado-form&alterar=$id' title='Alterar'>
                    <i class='fa fa-pencil'></i>
                </a>&nbsp
                <a  href=\"class/Monitoria.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir resultado do ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\">
