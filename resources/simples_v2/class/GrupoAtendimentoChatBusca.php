...
$filtros_query  = "WHERE nome LIKE '%".$letra."%' AND status = 1 ORDER BY nome ASC";
...
$dados = DBRead('','tb_grupo_atendimento_chat',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_grupo_atendimento_chat',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    foreach ($dados as $conteudo) {
        ...
        echo "<tr>";    
            echo "<td>$id</td>";
            echo "<td>$nome</td>";
            echo "<td><input type='color' class='form-control input-sm' value='$cor' disabled></td>";
            echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=grupo-atendimento-chat-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='/api/iframe?token=<?php echo $request->token ?>&view=grupo-atendimento-chat-vincular-operador&alterar=$id' title='Vincular operadores'><i class='fa fa-user'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/GrupoAtendimentoChat.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
