...
$letra = addslashes($parametros['opcao']);

// Informações da query
$filtros_query  = "WHERE opcao LIKE '%$letra%' AND status != '2' ORDER BY opcao ASC";
...
$dados = DBRead('','tb_tipo_falha_atendimento',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_tipo_falha_atendimento',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-7\">Opção</th>";
    echo "<th class=\"col-md-4\">Exibição</th>";
    echo "<th class=\"col-md-1 text-center\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_tipo_falha_atendimento'];
        $opcao = $conteudo['opcao'];
        $exibicao = $conteudo['exibicao'];
        ...
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=tipo-falha-atendimento-form&alterar=$id'\" style='cursor: pointer'>$opcao</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=tipo-falha-atendimento-form&alterar=$id'\" style='cursor: pointer'>$exibicao_nome[$exibicao]</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=tipo-falha-atendimento-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/TipoFalhaAtendimento.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
