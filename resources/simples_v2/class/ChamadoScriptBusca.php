...
$categoria = addslashes($parametros['categoria']);

// Informações da query
$filtros_query  = "INNER JOIN tb_categoria b ON a.id_categoria = b.id_categoria WHERE a.descricao LIKE '%".$letra."%' AND a.status = 1 ORDER BY a.id_chamado_script ASC";
...
$dados = DBRead('','tb_chamado_script a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_chamado_script a',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<thead>";
    echo "<tr>";
        echo "<th class=\"col-md-8\">Descrição</th>";
    ...
    foreach ($dados as $conteudo) {
        ...
        $id = $conteudo['id_chamado_script'];
        $descricao = $conteudo['descricao'];
        $categoria = $conteudo['nome'];

        echo "<tr>";    
            echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=categoria-form&alterar=$id'\" style='cursor: pointer'>$descricao</td>";
            echo "<td>".$categoria."</td>";

            echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=chamado-script-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/ChamadoScript.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
