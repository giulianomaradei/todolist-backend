...
$filtros_query = " WHERE nome LIKE '%$letra%' AND status = 1 ";
...
$dados = DBRead('', 'tb_patrimonio_localizacao', $filtros_query, "COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDOs
//
$dados = DBRead('', 'tb_patrimonio_localizacao', $filtros_query . " ORDER BY id_patrimonio_localizacao DESC LIMIT $inicio,$maximo");
if (!$dados) {
		...
		echo "<table class='table table-hover' style='font-size: 14px;'>";
			echo "<thead>";
				echo "<tr>";
					...
					echo "<th class=\"col-md-10 text-center\">Opções</th>";
				echo "</tr>";
			echo "</thead>";
			echo "<tbody>";
			foreach ($dados as $conteudo) {

				$id_patrimonio_localizacao = $conteudo['id_patrimonio_localizacao'];
				$nome = $conteudo['nome'];
				
				echo "<tr>";
				 	echo "<td>$id_patrimonio_localizacao</td>";
					echo "<td>$nome</td>";
					echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=patrimonio-localizacao-form&alterar=$id_patrimonio_localizacao' title='Alterar'><i class='fa fa-pencil'></i></a></td>";
