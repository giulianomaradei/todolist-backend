...
$filtros_query  = "WHERE nome_view LIKE '%$letra%' OR nome_pagina LIKE '%$letra%' ORDER BY nome_pagina ASC";
...
$dados = DBRead('','tb_pagina_sistema',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_pagina_sistema',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-6\">Nome da página</th>";
    echo "<th class=\"col-md-5\">Nome da view</th>";
    echo "<th class=\"col-md-1 text-center\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_pagina_sistema'];
        $nome_pagina = $conteudo['nome_pagina'];
        $nome_view = $conteudo['nome_view'];
        echo "<tr>";    
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pagina-sistema-form&alterar=$id'\" style='cursor: pointer'>$nome_pagina</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pagina-sistema-form&alterar=$id'\" style='cursor: pointer'>$nome_view</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=pagina-sistema-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/PaginaSistema.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome_pagina." (".$nome_view).")"."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
