...
$filtros_query  = "INNER JOIN tb_contrato_plano_pessoa b ON a.id_contrato_plano_pessoa = b.id_contrato_plano_pessoa INNER JOIN tb_plano c ON b.id_plano = c.id_plano INNER JOIN tb_pessoa d ON b.id_pessoa = d.id_pessoa WHERE d.nome LIKE '%$letra%' AND a.status = 1 ORDER BY d.nome ASC";
...
$dados = DBRead('','tb_contrato_recarga a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_contrato_recarga a',$filtros_query." LIMIT $inicio,$maximo", "a.*, b.*, d.nome AS 'nome', c.nome AS 'plano', c.cod_servico, b.nome_contrato");
...
if (!$dados) {
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-5\">Cliente - Contrato</th>";
    echo "<th class=\"col-md-3 text-center\">Data</th>";
    echo "<th class=\"col-md-3 text-center\">Quantidade de atendimentos</th>";
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_contrato_recarga'];
        $id_contrato_plano_pessoa = $conteudo['id_contrato_plano_pessoa'];
        $data_recarga = $conteudo['data_recarga'];
        $quantidade_atendimentos = $conteudo['quantidade_atendimentos'];
        $servico = getNomeServico($conteudo['cod_servico']);
        $nome = $conteudo['nome'];
        $nome_contrato = $conteudo['nome_contrato'];

        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=prazo-retorno-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "<td class='text-center'>".converteData($data_recarga)."</td>";
        echo "<td class='text-center'>".$quantidade_atendimentos."</td>";
        //echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=contrato-recarga-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/ContratoRecarga.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
