...
$filtros_query  = "INNER JOIN tb_usuario b ON a.id_usuario = b.id_usuario INNER JOIN tb_pessoa c ON b.id_pessoa = c.id_pessoa WHERE c.nome LIKE '%$letra%' ORDER BY a.data_de DESC";
...
$dados = DBRead('','tb_ferias a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_ferias a',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<th class=\"col-md-6\">Atendente</th>";
    ...
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_ferias'];
        $nome = $conteudo['nome'];
        $data_de = converteDataHora($conteudo['data_de']);
        $data_ate = converteDataHora($conteudo['data_ate']);
        echo "<tr>";
        echo "<td>$id</td>";
        echo "<td>$nome</td>";
        echo "<td>$data_de</td>";
        echo "<td>$data_ate</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=ferias-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/Ferias.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
