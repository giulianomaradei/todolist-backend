...
$filtros_query = " WHERE (nome LIKE '%$letra%' OR id_natureza_financeira_agrupador LIKE '%$letra%') ".$filtro;
...
// $dados = DBRead('', 'tb_natureza_financeira_agrupador', $filtros_query, "COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
//
$dados = DBRead('', 'tb_natureza_financeira_agrupador', $filtros_query . " ORDER BY nome ASC");
if (!$dados) {
			...
			foreach ($dados as $conteudo) {
                ...
                if ($status != 1) {
					...
					echo "<td class=\"text-center\">
						...
						<a href='/api/iframe?token=<?php echo $request->token ?>&view=natureza-financeira-agrupador-form&alterar=$id' title='Vizualizar'><i class='fa fa-eye'></i></a>&nbsp;&nbsp;&nbsp;";
						if($conteudo['status'] == 0){
							echo "<a href=\"class/NaturezaFinanceiraAgrupador.php?ativar=$id\" title='Ativar' onclick=\"if (!confirm('Ativar " . addslashes($nome) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-upload' style='color:#228B22;'></i></a>";

						}else if($conteudo['status'] == 1){
							echo "<a href=\"class/NaturezaFinanceiraAgrupador.php?desativar=$id\" title='Desativar' onclick=\"if (!confirm('Desativar " . addslashes($nome) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-download' style='color:#FFA500;'></i></a>";
						}	             
