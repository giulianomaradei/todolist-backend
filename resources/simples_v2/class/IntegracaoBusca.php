...
$filtros_query  = "INNER JOIN tb_contrato_plano_pessoa b ON a.id_contrato_plano_pessoa = b.id_contrato_plano_pessoa INNER JOIN tb_pessoa d ON b.id_pessoa = d.id_pessoa WHERE d.nome LIKE '%$letra%' ORDER BY d.nome ASC";
...
$dados = DBRead('', 'tb_integracao_contrato a', $filtros_query, "COUNT(*) AS 'num_registros'");
...
###################################################################################
// INICIO DO CONTEÚDO
$dados = DBRead('', 'tb_integracao_contrato a', $filtros_query . " LIMIT $inicio,$maximo", "a.*, b.*, d.nome AS 'nome_pessoa'");
if (!$dados) {
    ...
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_integracao_contrato'];
        $id_contrato_plano_pessoa = $conteudo['id_contrato_plano_pessoa'];
        $nome = $conteudo['nome_pessoa'];
        $id_integracao = $conteudo['id_integracao'];
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=integracao-form&alterar=$id&integracao=$id_integracao'\" style='cursor: pointer'>$nome</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=integracao-campos-default&contrato=$id_contrato_plano_pessoa' class='data-contrato' title='Valores default'><i class='fa fa-check-double' style='color:#008000;'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='/api/iframe?token=<?php echo $request->token ?>&view=integracao-form&alterar=$id&integracao=$id_integracao' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/Integracao.php?excluir=$id&id_contrato_plano_pessoa=$id_contrato_plano_pessoa\" title='Excluir' onclick=\"if (!confirm('Excluir " . addslashes($nome) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
