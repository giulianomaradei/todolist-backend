...
$dados = DBRead('','tb_horario_contrato a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_horario_contrato a',$filtros_query." LIMIT $inicio,$maximo", "a.*, b.*, d.nome AS 'nome', c.nome AS 'plano', c.cod_servico, b.nome_contrato");
if(!$dados){
    ...
    echo "<th class=\"col-md-5\">Tipo</th>";
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_horario_contrato'];
        ...
        $tipos = [
            ...
            "8" => "Horário que mandam as chamadas para a Belluno.",
            "9" => "Horários de atendimento via texto.",
            "10" => "Horários de retorno telefônico do provedor."
        ];
        $tipo = $tipos[$conteudo['tipo']];
        $servico = getNomeServico($conteudo['cod_servico']);
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=horario-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=horario-form&alterar=$id'\" style='cursor: pointer'>$tipo</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=horario-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/Horario.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
