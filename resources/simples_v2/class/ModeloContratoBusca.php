...
$filtros_query = "WHERE status != 0 ORDER BY nome_contrato ASC";
...
if (!$dados) {
	...
	echo "<div class='table-responsive'>";
		echo "<table class='table table-hover' style='font-size: 14px;'>";
			...
			//<a href=\"class/ModeloContrato.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($conteudo['nome_contrato'])."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a>
			foreach ($dados as $conteudo) {
				...
				if ($conteudo['status'] != 1) {
					...
					echo "<a href='/api/iframe?token=<?php echo $request->token ?>&view=modelo-contrato-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>";

					if($conteudo['status'] == 2){
						echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/ModeloContrato.php?ativar=$id\" title='Ativar' onclick=\"if (!confirm('Ativar " . addslashes($conteudo['nome_contrato']) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-upload' style='color:#228B22;'></i></a>";

					}else if($conteudo['status'] == 1){
						echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/ModeloContrato.php?desativar=$id\" title='Desativar' onclick=\"if (!confirm('Desativar " . addslashes($conteudo['nome_contrato']) . "?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-download' style='color:#FFA500;'></i></a>";
					}
					echo "</td>";
				echo "</tr>";
			}
