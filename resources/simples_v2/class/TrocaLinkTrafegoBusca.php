...
$filtros_query  = "INNER JOIN tb_link_acesso b ON b.id_link_acesso = a.id_link_acesso WHERE a.nome LIKE '%".$letra."%' OR b.nome LIKE '%".$letra."%' ORDER BY a.nome ASC";
...
$dados = DBRead('','tb_link_trafego a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_link_trafego a',$filtros_query." LIMIT $inicio,$maximo","a.*, b.nome AS nome_link");
...
if (!$dados) {
	...
	echo "<div class='table-responsive'>";
        ...
        foreach ($dados as $conteudo) {
            $id = $conteudo['id_link_trafego'];
            $nome = $conteudo['nome'];
            $nome_link = $conteudo['nome_link'];
            $ip = $conteudo['ip_trafego'];

            echo "<tr>";
                echo "<td> $nome</td>";
                echo "<td> $ip</td>";
                echo "<td> $nome_link</td>";
                echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=troca-link-trafego-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a></td>";
            echo "</tr>";
        }
            echo "</tbody>";
        echo "</table>";
