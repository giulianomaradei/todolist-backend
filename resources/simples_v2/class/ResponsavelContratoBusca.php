...
// Informações da query 
    ...
    $filtros_query  = "INNER JOIN tb_pessoa b ON a.id_pessoa = b.id_pessoa INNER JOIN tb_plano c ON a.id_plano = c.id_plano WHERE (b.nome LIKE '%$letra%' OR b.razao_social LIKE '%$letra%' OR c.nome LIKE '%$letra%' OR c.cod_servico LIKE '%$letra%' OR a.nome_contrato LIKE '%$letra%') $filtro AND a.status != 3 ORDER BY b.nome ASC, c.cod_servico ASC, a.data_inicio_contrato DESC";
...
if (!$dados) {
    ...
    foreach ($dados as $conteudo) {
        ...
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'>$id</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'><strong>$nome_pessoa";
        ...
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'>$servico - $nome_plano</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'>$status</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'>$nome_responsavel</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'>$nome_tecnico</td>";
        echo "<td class=\"text-center\" onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id'\" style='cursor: pointer; vertical-align: middle;'><a href='/api/iframe?token=<?php echo $request->token ?>&view=responsavel-contrato-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
