...
$dados = DBRead('','tb_erro_atendimento a ',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_erro_atendimento a ',$filtros_query." LIMIT $inicio,$maximo");
...
if (!$dados) {
    ...
    echo "<th class=\"col-md-7\">Usuário</th>";
    echo "<th class=\"col-md-1 text-center\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_erro_atendimento'];
        $nome = $conteudo['nome'];
        $atendente = $conteudo['email'];
        $nome_contrato = $conteudo['nome_contrato'];
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=erro-atendimento-cadastro-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=erro-atendimento-cadastro-form&alterar=$id'\" style='cursor: pointer'>$atendente</td>";
        
        $string_excluir = "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=erro-atendimento-cadastro-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>";
        if($tem_justificativa){
            $string_excluir .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/ErroAtendimento.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a>";
        }else{
            $string_excluir .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
        }
        $string_excluir .= "</td>";
