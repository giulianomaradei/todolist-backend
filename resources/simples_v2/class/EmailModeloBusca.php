...
$filtros_query  = "WHERE titulo LIKE '%$letra%' OR assunto LIKE '%$letra%' ORDER BY titulo ASC";
...
$dados = DBRead('','tb_email_modelo',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_email_modelo',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<thead>";
    echo "<tr>";
    echo "<th class=\"col-md-5\">Título</th>";
    echo "<th class=\"col-md-6\">Assunto</th>";
    echo "<th class=\"col-md-1 text-center\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_email_modelo'];
        ...
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=email-modelo-form&alterar=$id'\" style='cursor: pointer'>$titulo</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=email-modelo-form&alterar=$id'\" style='cursor: pointer'>$assunto</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=email-modelo-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/EmailModelo.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($titulo)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
