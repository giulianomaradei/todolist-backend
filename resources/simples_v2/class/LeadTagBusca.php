...
$filtros_query  = "WHERE descricao LIKE '%$letra%' ORDER BY descricao ASC";
...
$dados = DBRead('','tb_lead_tag',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_lead_tag',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    echo "<th class=\"col-md-10\">Descrição</th>";
    echo "<th class=\"col-md-1 text-center\">Opções</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_lead_tag'];
        $descricao = $conteudo['descricao'];
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=lead-tag-form&alterar=$id'\" style='cursor: pointer'>$id</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=lead-tag-form&alterar=$id'\" style='cursor: pointer'>$descricao</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=lead-tag-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/LeadTag.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($descricao)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
