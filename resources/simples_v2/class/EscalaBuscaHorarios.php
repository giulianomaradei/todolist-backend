...
$filtros_query  = "INNER JOIN tb_pessoa b ON a.id_pessoa = b.id_pessoa WHERE (a.id_perfil_sistema = '3' OR a.id_perfil_sistema = '15' OR a.id_perfil_sistema = '13') AND b.status != 2 AND a.status = 1 AND b.nome LIKE '%$letra%' ORDER BY b.nome ASC";
...
$usuario = DBRead('', 'tb_usuario a',"INNER JOIN tb_pessoa b ON a.id_pessoa = b.id_pessoa WHERE (a.id_perfil_sistema = '3' OR a.id_perfil_sistema = '15') AND a.id_usuario = '".$id_usuario."' AND a.status = 1 AND b.nome LIKE '%$letra%' ORDER BY b.nome ASC","a.id_usuario, b.nome, a.*, b.*");	
	    ...
	    echo "<tbody>";
		    ...
		    foreach ($dados as $atendente) {
		        ...
		        if($dados){
		        	...
		        	$visualizar  = "<td class=\"text-center\"><a style='cursor:pointer;' title='Editar' data-toggle='modal' data-target='#modal_".$atendente['id_usuario']."'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		             <a class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=exibe-escala-horarios&visualizar=$id' title='Visualizar'><i class='fa fa-eye'></i></a></td>";
		        	
		        }else{
		        	$visualizar = "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=escala-editar-horarios&alterar=$id' title='Inserir'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
		        }
		        echo $visualizar;
				    ...
				    <div class="modal-dialog">
				      ...
				      <div class="modal-content">
				        <div class="modal-header">
							...
							<div class="panel-title text-right pull-right"><a href="/api/iframe?token=<?php echo $request->token ?>&view=escala-editar-horarios&novo='.$atendente['id_usuario'].'"><button class="btn btn-xs btn-primary"><i class="fa fa-plus"></i> Nova</button></a></div>
				        </div>
				        <div class="modal-body" style="overflow: auto; max-height: 180px !important;">
							 ...
							 $dados_escala = DBRead('', 'tb_horarios_escala',"WHERE id_usuario = '".$id."' ORDER BY data_inicial DESC");
							 //sort($dados_escala);
							 ...
							 //style='overflow: auto; height: 565px !important;'
								...
								echo '<ul class="list-group">
				        			...
				        			foreach ($dados_escala as $conteudo_escala) {
				        				...
				        				$nome = $atendente['nome'];
				        				$id_escala = $conteudo_escala['id_horarios_escala'];
										if($data[1] == "01"){
											$mes = "Janeiro";
										}else if($data[1] == "02"){
											$mes = "Fevereiro";
										 ...
										 echo '
										  <li class="list-group-item d-flex justify-content-between align-items-center">
										    ...
										    <a style='text-align: center' href='/api/iframe?token=<?php echo $request->token ?>&view=escala-editar-horarios&alterar=$id&mes=".$data[1]."&ano=".$data[0]."' title='Editar ".$mes.'/'.$data[0]."' ><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/EscalaHorarios.php?excluir=$id_escala\" title='Excluir' onclick=\"if (!confirm('Excluir a escala de ".addslashes($data[1]."/".$data[0])." do(a) ".addslashes($nome)."?')) { return false; } \" title='Excluir'><i class='fa fa-trash' style='color:#b92c28;'></i></a>
