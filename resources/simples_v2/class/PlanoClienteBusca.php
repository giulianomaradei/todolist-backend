...
$dados = DBRead('','tb_plano_cliente_contrato a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_plano_cliente_contrato a',$filtros_query." LIMIT $inicio,$maximo", "a.*, b.*, d.nome AS 'nome', c.nome AS 'plano', c.cod_servico, b.nome_contrato");
if (!$dados) {
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_plano_cliente_contrato'];
        ...
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=plano-cliente-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=plano-cliente-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/PlanoCliente.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
