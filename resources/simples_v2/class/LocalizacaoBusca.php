...
$filtros_query  = "INNER JOIN tb_contrato_plano_pessoa b ON a.id_contrato_plano_pessoa = b.id_contrato_plano_pessoa INNER JOIN tb_plano c ON b.id_plano = c.id_plano INNER JOIN tb_pessoa d ON b.id_pessoa = d.id_pessoa INNER JOIN tb_localizacao e ON a.id_localizacao_contrato = e.id_localizacao_contrato WHERE d.nome LIKE '%$letra%' ORDER BY d.nome ASC";
...
$dados = DBRead('','tb_localizacao_contrato a',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
//
$dados = DBRead('', 'tb_localizacao_contrato a',$filtros_query." LIMIT $inicio,$maximo", "a.*, b.*, d.nome AS 'nome', c.nome AS 'plano', e.endereco, c.cod_servico, b.nome_contrato");
if (!$dados) {
    ...
    echo "<th class=\"col-md-5\">Endereço</th>";
    ...
    foreach($dados as $conteudo){
        ...
        $id = $conteudo['id_localizacao_contrato'];
        ...
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=localizacao-form&alterar=$id'\" style='cursor: pointer'>$nome";
        ...
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=localizacao-form&alterar=$id'\" style='cursor: pointer'>$endereco</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=localizacao-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/Localizacao.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
