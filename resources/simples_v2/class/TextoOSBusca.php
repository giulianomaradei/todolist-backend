...
$dados = DBRead('','tb_texto_os',$filtros_query,"COUNT(*) AS 'num_registros'");
...
// INICIO DO CONTEÚDO
// 
$dados = DBRead('', 'tb_texto_os',$filtros_query." LIMIT $inicio,$maximo");
if (!$dados) {
    ...
    foreach ($dados as $conteudo) {
        $id = $conteudo['id_texto_os'];
        $nome = $conteudo['nome'];
        echo "<tr>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=texto-os-form&alterar=$id'\" style='cursor: pointer'>$id</td>";
        echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=texto-os-form&alterar=$id'\" style='cursor: pointer'>$nome</td>";
        echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=texto-os-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"class/TextoOS.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir ".addslashes($nome)."?')) { return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
