...
if (!$dados) {
	...
	echo "<th class=\"col-md-4\">Razão Social</th>";
	echo "<th class=\"col-md-2\">Cidade</th>";
	echo "<th class=\"col-md-2\">Atualizado em</th>";
	...
	foreach ($dados as $conteudo) {
		...
		echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pessoa-form&alterar=$id'\" style='cursor: pointer'>$id";
		echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pessoa-form&alterar=$id'\" style='cursor: pointer'>$nome";
		...
		echo "</td>";
		echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pessoa-form&alterar=$id'\" style='cursor: pointer'>$razao_social</td>";
		echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pessoa-form&alterar=$id'\" style='cursor: pointer'>$cidade - $estado</td>";
		echo "<td onclick=\"window.location='/api/iframe?token=<?php echo $request->token ?>&view=pessoa-form&alterar=$id'\" style='cursor: pointer'>$data_atualizacao</td>";
		$disabled = "";
		if($id == 2){
			$disabled = "style='pointer-events:none; opacity: 0.4;'";
		}
		echo "<td class=\"text-center\"><a href='/api/iframe?token=<?php echo $request->token ?>&view=pessoa-form&alterar=$id' title='Alterar'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a $disabled href=\"class/Pessoa.php?excluir=$id\" title='Excluir' onclick=\"if (!confirm('Excluir " . addslashes($nome) . "?')) {  return false; } else { modalAguarde(); }\"><i class='fa fa-trash' style='color:#b92c28;'></i></a></td>";
		echo "</tr>";
	}
	echo "</tbody>";
	echo "</table>";
